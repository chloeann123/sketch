...
//set the location and filename of our uploaded files, filename is how many milliseconds have passed since january 1 1970.
  ...
  let uploadLocation = __dirname + '/public_html/uploads/' + Date.now() + '.png'  // where to save the file to. make sure the incoming name has a .wav extension
  ...
  //send the filename down to the gallery for later viewing!
  io.emit('newDrawing', Date.now() + '.png' )
...
io.on('connection', function(socket){
  ...
  socket.on('getFileCount', function(){

    console.warn('make sure there are no .DS_Store files in the uploads folder!')

    fs.readdir(__dirname + '/public_html/uploads/', function(err, files){

      io.emit('fileCount', files)
    })
